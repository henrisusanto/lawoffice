...
function sc_button($atts, $content=null){	
	...
	$output = '<div class="sc_button sc_button_style_' . $style . ' sc_button_size_' . $size . ($align && $align!='none' ? ' align'.$align : '') . (!empty($class) ? ' '.$class : '')
...
function sc_contact_form($atts, $content = null) {
	...
	$THEMEREX_sc_contact_form_counter = 0;
	$content = do_shortcode($content);
	$form_type = (!$content &&( $style == '1')) ? 'contact_1' : 'contact_2';  //contact_2 = phone instead of subj
	$output = '<div ' . ($id ? ' id="' . $id . '_' . $form_type . '"' : '') . ' class="sc_contact_form sc_contact_form_'.$form_type.(!empty($class) ? ' '.$class : '').'"'.($s!='' ? ' style="'.$s.'"' : '') .'>'
		...
		. ($content != '' 
			? $content 
			: ($style == '1'
					?   '<div class="columnsWrap">' // Contact form with subject - standart
					...
					.'</div>'
					.'<div class="columns1">'
					.'<input id="sc_contact_form_subj" type="text" name="subject" placeholder="' . __('Subject', 'themerex') . '">'
					.'</div>'
					.'</div>'
					.'<div class="message">'
					...
					.'<div class="squareButton sc_button_size_huge sc_button_style_global global huge"><a href="#" class="sc_contact_form_submit">' . __('Submit', 'themerex') . '</a></div>'
					.'</div>'
/*if statement */   :'<div class="columnsWrap">' // Contact form with phone instead of subject
					...
					.'</div>'
					.'<div class="columns1_1">'
					.'<input id="sc_contact_form_phone" type="text" name="phone" placeholder="' . __('Phone (Ex.0123456789)', 'themerex') . '">'
					.'</div>'
					.'</div>'
					.'<div class="message">'
					.'<textarea id="sc_contact_form_message" class="textAreaSize" name="message" placeholder="' . __('Message', 'themerex') . '"></textarea>'
					.'</div>'
					.'<div class="sc_contact_form_button">'
					.'<div class="squareButton sc_button_style_global global"><a href="#" class="sc_contact_form_submit">' . __('Submit your message', 'themerex') . '</a></div>'
					.'</div>'
				)
			)
...
function sc_emailer($atts, $content = null) {
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '').' class="sc_emailer inputSubmitAnimation' . ($align && $align!='none' ? ' sc_align' . $align : '') . (sc_param_is_on($open) ? ' sFocus rad4 opened' : ' radCircle') . (!empty($class) ? ' '.$class : '') . '"' . ($s ? ' style="'.$s.'"' : '') . '>'
...
function sc_price_table($atts, $content=null){	
	...
	$THEMEREX_sc_price_table_counter = 0;
	$count = max(1, $count);
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_pricing_table columns_' . $count . ($align && $align!='none' ? ' align'.themerex_strtoproper($align) : '') . (!empty($class) ? ' '.$class : '') . '"' . ($s!='' ? ' style="'.$s.'"' : '') . '>'
			. do_shortcode($content)
		. '</div>';
...
function sc_price_item($atts, $content=null) {
	...
	$output = '<div class="sc_pricing_columns sc_pricing_column_'.$THEMEREX_sc_price_table_counter. (!empty($class) ? ' '.$class : '').'"><ul'.(sc_param_is_on($animation) ? ' class="columnsAnimate"' : '') . ($id ? ' id="' . $id . '"' : '') . '>'
		. do_shortcode($content) 
		. '</ul></div>';
	...
	if (!empty($name) && !empty($position)) {
		$output = '<div class="columns1_'.$THEMEREX_sc_team_count . (!empty($class) ? ' '.$class : '').'">'
				. '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_team_item sc_team_item_' . $THEMEREX_sc_team_counter . ($THEMEREX_sc_team_counter % 2 == 1 ? ' odd' : ' even') . ($THEMEREX_sc_team_counter == 1 ? ' first' : '') . '">'
					...
					. '<h3 class="sc_team_item_title">' . (strlen($link) > 0 ? '<a href="' . $link . '">' . $name . '</a>' : '' . $name . '') . '</h3>'
