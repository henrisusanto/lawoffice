...
class TRX_Importer {
    ...
    var $options = array(
        ...
        'file_with_royal_sliders' => '/admin/tools/importer/data/royal_sliders.txt',	// Name of file with sliders data

        'domain_dev'		=> '.dnw',						// Domain on developer's server
        'domain_demo'		=> '.com',						// Domain on demo-server
        'demo_url'          => 'http://lawoffice.themerex.net/', // URL of the demo site - need for change URLs in the custom menu items
        ...
        'page_on_front'		=> 'Homepage (Law Office)',					// Homepage title
                    ...
                    <form id="trx_exporter_form" action="#" method="post">
                        ...
                        <div class="trx_buttons">
                            <?php if ($this->export_options!='') { ?>
                                ...
                                <textarea rows="10" cols="80"><?php echo esc_html($this->export_sliders); ?></textarea>
                            <?php } else { ?>
    ...
    function clear_tables() {
        ...
        if ($this->options['overwrite_content']) {
            ...
            if ($this->options['import_sliders']) {
                $res = $wpdb->query("TRUNCATE TABLE " . esc_sql($wpdb->prefix) . "new_royalsliders");
                if ( is_wp_error( $res ) ) echo __( 'Failed truncate table "new_royalsliders".', 'themerex' ) . ' ' . trim($res->get_error_message()) . '<br />';
            }
        }
    }
    ...
    function import_sliders() {
        ...
        if (!empty($this->options['file_with_royal_sliders'])) {
            ...
            global $wpdb;
            $sliders_txt = themerex_fgc(themerex_get_file_dir($this->options['file_with_royal_sliders']));
