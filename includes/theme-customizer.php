...
function prepareThemeCustomStyles() {
	...
	// Theme color from customizer - f55e45
	$clr = '';
	if ($customizer)
...
.mejs-embed, .mejs-embed body, .mejs-container .mejs-controls { background:'.$clr.' !important; }
.mejs-controls .mejs-volume-button .mejs-volume-slider { background: rgba('.$rgb['r'].','.$rgb['g'].','.$rgb['b'].',0.7) !important; }

.top_panel_above .fullScreenSlider .topWrap,
.top_panel_above .fullScreenSlider .topWrap .topMenuStyleLine > ul > li ul,
.top_panel_above .fullScreenSlider .topWrap .topMenuStyleLine > ul > li .menu-panel { background-color: rgba('.$rgb['r'].','.$rgb['g'].','.$rgb['b'].',0.8);}

.sc_slider_flex .sc_slider_info,
.sc_slider_swiper .sc_slider_info,
.sc_slider_flex .flex-direction-nav li,
.sc_slider_swiper .flex-direction-nav li { background-color:rgba('.$rgb['r'].','.$rgb['g'].','.$rgb['b'].',0.8) !important; }
...
.topTabsWrap { background: ' . $clr2 . '; }
		';
		$custom_style = apply_filters('theme_skin_set_menu_bgcolor', $custom_style, $clr, $rgb);
	}

	// Top panel fore color - b9b9aa
	$clr3 = '';
	if ($customizer)
		$clr3 = getValueGPC('user_menu_color', '');
	if (empty($clr3))
		$clr3 = get_custom_option('user_menu_color');
	if (!empty($clr3)) {
		$clr = $clr3;
		$custom_style .= '
			.booking_day_black a > div,
			 .days_container_all .booking_day_number,
			 .days_container_all .booking_day_book  {color: ' . $clr . ' !important;}
			.hover_red .sc_section .sc_title,
			.squareButton.light.ico > a::after, .squareButton.light.ico > a::before,
			.userHeaderSection .sc_title_icon:before {color: ' . $clr . ';}
		';		
